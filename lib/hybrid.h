  ...
  static bool SplitLattice(const std::vector<unsigned>& parts,
    ...
    for (const auto& gate : gates) {
      if (gate.kind == gate::kMeasurement) {
        IO::errorf("measurement gates are not suported by clfsimh.\n");
        return false;
      }

      if (gate.controlled_by.size() > 0) {
        IO::errorf("controlled gates are not suported by clfsimh.\n");
        return false;
      }
      ...
      case 2:  // Two qubit gates.
        ...
        break;
      default:
        IO::errorf("multi-qubit gates are not suported by clfsimh.\n");
        return false;
      }
    }
